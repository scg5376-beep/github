...
S1(도구 구성)에서 **"코덱스로만 작업한다"** 를 고른 경우입니다.
...
```bash
# 0) 환경 확인 — 도구 구성(S1) + GitHub 연결(S2)
./mp setup                      # ❌ 가 뜨면 그 항목부터 사용자에게 질문
                                #    레포가 없다면 ./mp setup --guide
...
1. `./mp setup` 으로 0단계(S1 도구 구성 · S2 GitHub 연결)를 먼저 통과시키고,
   그다음 `profile.yaml` 을 읽어 **Q0 → Q1~Q4** 순서로 묻는다.

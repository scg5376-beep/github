...
## 룩 단위 정리
`./mp organize` 는 룩북 카드 전체뿐 아니라 **개별 룩(L1, L2 …)의 사용 이력도** 올립니다.
그래서 "룩북은 자주 쓰는데 이 룩만 안 쓴다" 는 경우를 따로 잡아냅니다.

```bash
./mp audit --type lookbook
```
리포트의 **"안 쓰이는 룩"** 표를 보고, 룩북 카드의 `looks` 목록에서 해당 항목만 지우면 됩니다.
카드 전체를 보관할 필요는 없습니다. **지우기 전에 반드시 사용자에게 확인하세요.**

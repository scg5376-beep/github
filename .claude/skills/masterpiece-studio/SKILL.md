...
질문은 **두 단계**다. `./mp setup` 으로 0단계를 먼저 통과시킨다.
...
| # | 묻는 것 | 답이 정하는 것 |
|---|---|---|
| **S1** | CODEX로만? 클로드코드와 혼합? | `codex`→**§3 SOLO** / `mixed`→**§4 RELAY**. 모르면 SOLO 권함 |
| **S2** | 작업물을 어느 레포에? | 모르면 **`./mp setup --guide`** — 레포 생성부터 안내 |
| **Q0** | 5종을 어떤 형태로 쓰는가 | `profile.yaml` 에 저장. 이후엔 확인 한 줄 |
| **Q1** | 제공 / 생성 | 기존 카드 검색 / `new_masterpiece.py` |
| **Q2** | 자산축적 / 작업만 | 카드·레시피 커밋 / outputs만 |
| **Q3** | 저장 폴더 | 무응답 → `outputs/_unsorted/미정` |
| **Q4** | 자동매칭 / 수동 | `--mode auto` / `--mode manual` |

- **0단계가 통과하기 전에는 아무것도 만들지 않는다.**
- Q1~Q4는 묶어서 물어도 되지만 **전부 답을 받기 전에는 생성 단계로 가지 않는다.**
- 사용자가 문장 안에 이미 답한 항목은 확인만 하고 넘어간다.
- RELAY에서는 Claude가 답을 `--answers` 로 오더에 실어 보낸다. **CODEX는 다시 묻지 않는다.**

> 질문 문구·분기·무응답 처리의 **상세는 `references/00-question-protocol.md`**.
> 이 파일에는 요약만 둔다(같은 규칙을 두 곳에 쓰지 않는다).

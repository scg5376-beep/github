...
3. **실행 모드(Q1)를 정하기 전에는 아무것도 만들지 않는다.** 모드에 따라 이후 절차가 갈린다.
...
> - SOLO — 빠르고 단순. 몇 컷 뽑을 때
> - RELAY — 지시/검수가 기록으로 남음. 대량·반복·팀 작업

| 답 | `run_mode` | 이후 절차 |
|---|---|---|
| SOLO / 코덱스만 / 한번에 | `solo` | **§3 SOLO 절차** |
| RELAY / 클로드가 지시 / 릴레이 | `relay` | **§4 RELAY 절차** |

정해지면 `profile.yaml`에 저장하고, 이후 세션에서는 확인만 한다.

### 그다음 반드시 물어야 할 4가지 (모드 공통)

| # | 질문 | 선택지 | 결과 |
|---|---|---|---|
| **Q2** | 마스터피스를 **제공**하시겠어요, **새로 만들**까요? | `제공` / `생성` | 제공→기존 카드 검색·등록, 생성→`new_masterpiece.py` |
| **Q3** | 작업이 끝날 때마다 **자산으로 쌓을까요**, **작업만 할까요**? | `자산축적` / `작업만` | 자산축적→카드+레시피 커밋, 작업만→outputs만 남김 |
| **Q4** | CODEX로 만든 이미지를 **어느 폴더**에 저장할까요? | 경로 입력 / 무응답 | 무응답 → `outputs/_unsorted/미정` 에 커밋 |
| **Q5** | **AI가 자동매칭**할까요, **수동으로 지정**하실래요? | `자동` / `수동` | `--mode auto` / `--mode manual` |

> Q2~Q5는 한 번에 묶어서 물어도 되지만, **전부 답을 받기 전에는 생성 단계로 넘어가지 않는다.**
> 사용자가 이미 문장 안에 답을 준 항목은 확인만 하고 넘어간다.
> RELAY 모드에서는 Claude가 받은 답을 `--answers` 로 오더에 실어 보내므로 **CODEX는 다시 묻지 않는다.**
...
2. `run_mode` 확인 → `ask` 면 SOLO/RELAY를 묻는다 (Q1)
3. Q2~Q5 중 문장에서 안 채워진 것만 질문
